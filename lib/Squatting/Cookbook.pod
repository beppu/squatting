...
to the user.  Wikipedia's approach of using ISO-639-1 language tags in the
subdomains of wikipedia.org is my favorite way of doing this.  For example,
the English version of Wikipedia is at L<http://en.wikipedia.org/> and the
Korean version of Wikipedia is at L<http://ko.wikipedia.org/>.  
...
=item The URLs look nice and are easy to manage.
...
C<service()> method can be overridden as follows:
  ...
  sub service {
    my ($app, $c, @args) = @_;
    $c->v->{tr} = language_settings($c);
    $app->next::method($c, @args);
  }

The important code is in the C<language_settings()> function.

=head3 Creating Localizations with Locale::Maketext
