...
Please excuse the lack of polish on this document.  It is a work in progress.

=head2 The Anatomy of a Squatting Application

This section describes the structure of a simple Squatting app.
For the sake of our examples, please assume the following:

=over 4

=item The name of the app is "App".

It may lack of creativity, but it makes things easier.

=item C<$App> eq "App".

This is also used to emphasize that this is a general principle that works
for all apps.
...
to change the values in C<%CONFIG> from the command line.  To see a demonstration
of this, try the following commands:

  squatting Pod::Server
  squatting Pod::Server -c background_color='#301'

Notice how the background color is a dark blue in the first example, but when
we pass in a custom background_color, the background color is a dark red.

=head3 Make a Package for Your Controllers

Next, create a package for your controllers.

=over 4

=item The package name should be $App . "::Controllers".

This package name is plural, because we're using the package not as a class
definition but as a namespace for containing controllers.

=item The package should C<use Squatting ':controllers'>.

Using L<Squatting> this way will export two functions:  C<R()> for URL
generation, and C<C()> as a shortcut for creating L<Squatting::Controller>
objects.

=item The package should have a package variable named C<@C>.
...
use as of this writing.  This is how you'd implement a view for it.
  ...
  our @V = (
    V(
      ...
      _ => sub {
        ...
        my $tt = $self->{tt};
        $v->{self} = $self;
        $tt->render(":$self->{template}", $v);
      },
    ),
  );
  1;

As you can see, it's structured very similarly to the Tenjin view in the
previous example.
